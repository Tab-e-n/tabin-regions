...
func _ready():
	var packed_map : PackedScene = load(MapSetup.current_directory + "/" + MapSetup.current_map_name)
	if packed_map:
	...
	for i in range(align_size):
		...
		leader.frame = 0
		AlignmentList.color_leader(leader, current_map.get_alignment_color(i + 1))
...
func choose_align(align : int):
	...
	for i in range(MapSetup.player_amount):
		if player_alignments[i] == 0:
			...
			leader.frame = 0
			AlignmentList.color_leader(leader, current_map.get_alignment_color(align))
